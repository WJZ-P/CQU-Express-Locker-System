...
> **说明**: 
> - 若 token 有效，返回 code 200，并刷新 token（延长有效期），返回新 token
> - 若 token 无效或过期，返回 code 401，valid 为 false
> - expiresIn 单位: 秒
...
> **说明**: 此接口用于显示某个快递柜各类型格口的实时可用数量，便于用户在创建寄存订单时了解可选择的格口情况
...
| 认证 | POST /auth/send-code | ✅ 已完成 |
| 认证 | POST /auth/register | ✅ 已完成 |
| 认证 | POST /auth/verify-token | ✅ 已完成 |
| 用户 | GET /user/profile | ✅ 已完成 |
| 用户 | PUT /user/profile | ✅ 已完成 |
| 快递 | GET /express/pending | ✅ 已完成 |
| 快递 | GET /express/{expressId} | ✅ 已完成 |
| 快递 | POST /express/pickup | ✅ 已完成 |
| 快递 | POST /express/open | ✅ 已完成 |
| 快递 | POST /express/send | ✅ 已完成 |
| 寄存 | POST /storage/create | ✅ 已完成 |
| 寄存 | GET /storage/list | ✅ 已完成 |
| 快递柜 | GET /locker/list | ✅ 已完成 |
| 快递柜 | GET /locker/{lockerId} | ✅ 已完成 |
| 快递员 | GET /courier/profile | ✅ 已完成 |
| 快递员 | POST /courier/deliver | ✅ 已完成 |
| 快递员 | GET /courier/query-receiver | ✅ 已完成 |
| 快递员 | GET /courier/pending-collect | ✅ 已完成 |
| 快递员 | GET /courier/pending-return | ✅ 已完成 |
| 快递员 | POST /courier/open-compartment | ✅ 已完成 |
| 历史 | GET /history | ✅ 已完成 |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-01-25 | v1.1.0 | 所有接口已实现完成，包括寄存模块、快递员模块和历史记录模块 |
